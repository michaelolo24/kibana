 ...
 * writer lifecycle, reconciliation + reset task registration,
 * per-space data-view ensure, and administrator route registration.
 *
...
export class CasesAnalyticsV2Service {
   ...
   * and reset task runners (they need an SO client without a request
   * context).
   */
   ...
   * and the reset task registered in `setup()` close over callbacks
   * that read this — they need the start contract, but registration
   * happens in setup where only the setup contract is available.
   */
  ...
  /**
   * Plugin setup hook. Registers the reconciliation and reset task
   * types with Task Manager and the administrator routes with the
   * router. Task definitions and route handlers must exist before any
   * node accepts requests, so this runs in `setup()`.
   *
   * Runtime deps (SO client, writers, TM start contract) aren't
  ...
  public setup(deps: CasesAnalyticsV2SetupDeps): void {
    ...
    // contract, internal SO client, writers) are resolved at call time
