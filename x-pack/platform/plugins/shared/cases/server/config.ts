...
export const ConfigSchema = schema.object({
  ...
  analyticsV2: schema.object({
     ...
     * full re-walk of every case and every user-action across every
     * space, which is `O(documents)` not `O(spaces)`. At small/medium
     * tenant sizes (≤ ~2K spaces) the default 60 minutes is comfortable.
     * At larger tenants it must be raised — measured wall-clock at 10K
     * spaces with default `resetPageDelayMs` is roughly 75 minutes for
     * the activity surface alone, so 240 minutes is a safe target with
     * margin. The 24-hour ceiling (`max: 1440`) prevents typo-driven
     * zombie tasks from holding a Task Manager slot indefinitely.
     *
     ...
     * fast as possible — the right behaviour for most operators, who
     * just want `/reset` to converge.
     *
     * Operators on shared / capacity-constrained ES clusters can raise
     * this to throttle the bulk-write pressure the backfill puts on the
     * cluster. At 10K spaces / ~15M activity docs:
     *   - `0`   → ~75 minutes wall-clock, ~3,300 docs/sec sustained ES write rate
     *   - `50`  → ~200 minutes, ~1,250 docs/sec
     *   - `100` → ~325 minutes, ~770 docs/sec
     * The trade-off is wall-clock vs cluster friendliness — a backfill
     * is a one-shot operation, so accepting a longer wall-clock to be
     * a better neighbour to other workloads is often the right call on
     * busy clusters.
     *
