...
export const useInitFlyoutFromUrlParam = () => {
  ...
  const dataTableCurrent = useShallowEqualSelector((state) =>
    getDataTable(state, TableId.alertsOnAlertsPage)
  );
  ...
  const loadExpandedDetailFromUrl = useCallback(() => {
    if (dataTableCurrent && urlDetails) {
      const { initialized, indexNames, isLoading, totalCount } = dataTableCurrent;
      const tableExists = initialized && indexNames.length > 0 && !isLoading && totalCount > 0;
      const hasFlyout = dataTableCurrent.expandedDetail?.query?.panelView !== urlDetails?.panelView;
      if (tableExists && hasFlyout) {
