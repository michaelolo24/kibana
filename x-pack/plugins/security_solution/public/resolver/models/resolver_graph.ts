 ...
 */
export function relatedEventsStats(graph: ResolverGraph): Map<string, EventStats> {
  const nodeStats: Map<string, EventStats> = new Map();

  for (const node of graph.nodes) {
    if (node.stats && typeof node?.nodeId === 'string') {
      nodeStats.set(node.nodeId, node.stats);
    }
  }
  return nodeStats;
}
