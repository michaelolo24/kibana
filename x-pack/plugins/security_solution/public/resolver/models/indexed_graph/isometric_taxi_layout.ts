...
export function isometricTaxiLayoutFactory(indexedGraph: IndexedGraph): IsometricTaxiLayout {
   ...
   */
  const widths: Map<ResolverGraphNode, number> = widthsOfNodeSubtrees(indexedGraph);
   ...
   */
  const positions: Map<ResolverGraphNode, Vector2> = nodePositions(indexedGraph, widths);
...
function ariaLevels(indexedGraph: IndexedGraph): Map<ResolverGraphNode, number> {
  ...
  for (const node of indexedGraphModel.levelOrder(indexedGraph)) {
    const parentNode = indexedGraphModel.connection(indexedGraph, node);
 ...
 *
 */
function widthsOfNodeSubtrees(indexedGraph: IndexedGraph): ProcessWidths {
  ...
  const nodesInReverseLevelOrder: SafeResolverEvent[] = [
    ...indexedGraphModel.levelOrder(indexedGraph),
  ].reverse();

  for (const node of nodesInReverseLevelOrder) {
    const children = indexedGraphModel.connections(indexedGraph, eventModel.nodeID(node));
...
function processEdgeLineSegments(
  ...
  for (const metadata of levelOrderWithWidths(indexedGraph, widths)) {
    ...
    const { node, connection, connectionWidth } = metadata;
    const position = positions.get(node);
    const connectionPosition = positions.get(connection);
    const connectionID = eventModel.nodeID(connection);
    ...
    const edgeLineID = `edge:${connectionID ?? 'undefined'}:${nodeID}`;

    if (position === undefined || connectionPosition === undefined) {
      /**
    ...
    const parentTime = eventModel.timestampSafeVersion(parent.data);
    const processTime = eventModel.timestampSafeVersion(process.data);
     ...
     */
    const midwayY = connectionPosition[1] + (position[1] - connectionPosition[1]) / 2;
    ...
    const siblings = indexedGraphModel.children(
      indexedGraph,
    ...
    if (metadata.isOnlyChild) {
      ...
      edgeLineSegments.push({ points: [connectionPosition, position], metadata: edgeLineMetadata });
      ...
      const lineFromParentToMidwayLine: EdgeLineSegment = {
        points: [connectionPosition, [connectionPosition[0], midwayY]],
        metadata: { reactKey: `parentToMid${edgeLineID}` },
      };

      const widthOfMidline = connectionWidth - firstChildWidth / 2 - lastChildWidth / 2;

      const minX = connectionWidth / -2 + firstChildWidth / 2;
      ...
      const midwayLine: EdgeLineSegment = {
        points: [
          [
            ...
            connectionPosition[0] + minX,
            midwayY,
          ],
          [
            // Position line relative to the parent's x component
            connectionPosition[0] + maxX,
            midwayY,
          ],
        ],
...
function nodePositions(indexedGraph: IndexedGraph, widths: ProcessWidths): ProcessPositions {
   ...
   * reset the counters.
   */
  let lastProcessedConnectionNode: ResolverGraphNode | undefined;
  /**
  ...
  for (const metadata of levelOrderWithWidths(indexedGraph, widths)) {
    ...
    if (metadata.parent === null) {
      ...
      const position = vector2.add([xOffset, yDistanceBetweenNodes], connectionPosition);
...
function* levelOrderWithWidths(
  ...
  for (const node of indexedGraphModel.levelOrder(graph)) {
    const connection = indexedGraphModel.connection(graph, node);
    ...
    if (connection === undefined) {
      ...
      const connectionWidth = widths.get(connection);

      if (connectionWidth === undefined) {
        /**
         * All widths have been precalcluated, so this will not happen.
         */
        throw new Error();
      ...
      const siblings = indexedGraphModel.connections(graph, eventModel.nodeID(connection));
...
export function nodePosition(model: IsometricTaxiLayout, nodeID: string): Vector2 | undefined {
  ...
  for (const candidate of model.processNodePositions.keys()) {
    if (eventModel.nodeID(candidate) === nodeID) {
