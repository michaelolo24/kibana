...
function ariaLevels(indexedGraph: IndexedGraph): Map<ResolverGraphNode, number> {
  ...
  for (const node of indexedGraphModel.levelOrder(indexedGraph)) {
    // TODO: For simplicity, we currently only get one parent so just grab it. Refactor later
    const parentNode = indexedGraphModel.parents(indexedGraph, node)?.[0];
...
function widthsOfNodeSubtrees(indexedGraph: IndexedGraph): ProcessWidths {
  ...
  for (const node of nodesInReverseLevelOrder) {
    const children = indexedGraphModel.neighbors(indexedGraph, eventModel.nodeID(node));
...
function processEdgeLineSegments(
  ...
  for (const metadata of levelOrderWithWidths(indexedGraph, widths)) {
    ...
    const parentID = eventModel.nodeID(parent);
    ...
    const parentTime = eventModel.timestampSafeVersion(
      parent.data as { '@timestamp': ECSField<number> }
    );
    const processTime = eventModel.timestampSafeVersion(
      node.data as { '@timestamp': ECSField<number> }
    );
    ...
    const siblings = indexedGraphModel.neighbors(indexedGraph, eventModel.nodeID(parent));
...
function* levelOrderWithWidths(
  ...
  for (const node of indexedGraphModel.levelOrder(indexedGraph)) {
    ...
    if (parentNode === undefined) {
      ...
      const parentWidth = widths.get(parentNode);
      ...
      const metadata: Partial<ProcessWithWidthMetadata> = {
        node,
        width,
        parent: parentNode, // TODO: This may become an array in the future
        parentWidth, // TODO: this may be an array in the future
      };

      const siblings = indexedGraphModel.neighbors(indexedGraph, eventModel.nodeID(parentNode));
...
export function nodePosition(model: IsometricTaxiLayout, nodeID: string): Vector2 | undefined {
  ...
  for (const candidate of model.processNodePositions.keys()) {
    if (eventModel.nodeID(candidate) === nodeID) {
      return model.processNodePositions.get(candidate);
    }
  }
}
