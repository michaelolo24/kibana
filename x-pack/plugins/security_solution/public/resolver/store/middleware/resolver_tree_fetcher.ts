 ...
 * In an effort to prepare for those scenarios, the `neighbors` field will serve as an array of all nodes directly connected to this one.
...
const convertNodesToResolverGraphFormat = (
  ...
  return data.reduce((resolverNodes: ResolverGraphNode[], current: ResolverNode) => {
    ...
    const neighbors = current.data[schema.parent] as string[];

    if (!nodeId || !Array.isArray(nodeId)) return resolverNodes;
    if (!neighbors || !Array.isArray(neighbors)) return resolverNodes;
...
export function ResolverTreeFetcher(
  ...
  return async () => {
    ...
    // TODO: Get timeline from selector.
    ...
    if (selectors.treeRequestParametersToAbort(state) && lastRequestAbortController) {
      ...
      if (result !== undefined) {
        const resolverGraph: ResolverGraph = {
          // TODO: Get backend to send back nodeOrigin to replace this field and maybe do the conversion on the backend?
